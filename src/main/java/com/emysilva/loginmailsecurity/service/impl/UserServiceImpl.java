...
import com.emysilva.loginmailsecurity.exception.EmailExistException;
import com.emysilva.loginmailsecurity.exception.TokenException;
import com.emysilva.loginmailsecurity.exception.TokenNotFoundException;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public ConfirmationToken signup(SignupRequest signupRequest) throws MessagingException {
        boolean emailExist = userRepository.findByEmail(signupRequest.getEmail()).isPresent();
        boolean usernameExist = userRepository.findByName(signupRequest.getUsername()).isPresent();

        String token = UUID.randomUUID().toString();

        if (emailExist) throw new EmailExistException("Email has already been taken");
        if (usernameExist) throw new EmailExistException("Username has already been taken");
        ...
        ConfirmationToken confirmationToken = new ConfirmationToken(
                token, LocalDateTime.now(), LocalDateTime.now().plusMinutes(1), user
        );
        confirmationTokenService.saveConfirmationToken(confirmationToken);

        //send email
        emailSenderService.sendEmail(signupRequest.getEmail(), buildEmail(user.getName(),
